...
const ExportSection = ({ svgRef }) => {
    ...
    return (
      <div className="border-t border-gray-200 pt-3 mt-4">
        <h4 className="text-sm font-medium text-gray-700 mb-2">Скачать карту</h4>
...
const ExportButton = ({ onClick, disabled, loading, icon, text, color }) => {
    return (
        <button
        onClick={onClick}
        disabled={disabled}
        className={`flex-1 px-3 py-2 text-xs font-bold ${color} text-white rounded hover:bg-blue-600 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
        >
        {loading ? (
